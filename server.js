...
module.exports = async (client) => {
        ...
        if (banned) {
            client.users.fetch(req.user.id).then(async a => {
                client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({
                    ...
                    dynamic: true
                })).setColor("RED").setDescription(`[**${a.username}**#${a.discriminator}](https://xottikmwgg.drmizayt.repl.co/user/${a.id}) logged into the site.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
            })
            req.session.destroy(() => {
                res.json({
                    login: false,
            ...
            try {
                ...
                request({
                    url: `https://discordapp.com/api/v10/guilds/${config.server.id}/members/${req.user.id}`,
                    method: "PUT",
                    json: {
            ...
            client.users.fetch(req.user.id).then(async a => {
                client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({
                    ...
                    dynamic: true
                })).setColor("GREEN").setDescription(`[**${a.username}**#${a.discriminator}](https://xottikmwgg.drmizayt.repl.co/user/${a.id}) Just Logged into the site.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
    ...
    app.get("/dc", (req, res) => {
        res.redirect('https://discord.gg/B397UAD2NN');
    });
    ...
    app.get("/robots.txt", function(req, res) {
        ...
        res.send(`Sitemap: https://xottikmwgg.drmizayt.repl.co/sitemap.xml`);
    });

    app.get("/sitemap.xml", async function(req, res) {
        let link = "<url><loc>https://xottikmwgg.drmizayt.repl.co/</loc></url>";
        let botdataforxml = await botsdata.find()
        botdataforxml.forEach(bot => {
            link += "\n<url><loc>https://xottikmwgg.drmizayt.repl.co/bot/" + bot.botID + "</loc></url>";
        })
    ...
    app.post("/uptime/add", checkMaintence, checkAuth, async (req, res) => {
        ...
        if (!rBody['link']) {
            ...
            if (dd.length > 25) res.redirect('?error=true&message=Your uptime limit has reached.')
            ...
            client.users.fetch(req.user.id).then(a => {
                client.channels.cache.get(channels.uptimelog).send(new Discord.MessageEmbed()
                    ...
                    .addField("Uptime Limit;", `${dd.length+1}/25`, true)
                )
                new uptimedata({
    ...
    app.post("/admin/addcode", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.codelog).send(new Discord.MessageEmbed()
            ...
            .setDescription(`The user named **[${req.user.username}](https://xottikmwgg.drmizayt.repl.co/user/${req.user.id})** added the code named **${rBody['codename']}** to the system.`)
            .addField("Code Link", `https://xottikmwgg.drmizayt.repl.co/code/${kod}`, true)
    ...
    app.post("/admin/editcode/:code", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.codelog).send(new Discord.MessageEmbed()
            ...
            .setDescription(`The user named **[${req.user.username}](https://xottikmwgg.drmizayt.repl.co/user/${req.user.id})** edited the code named **${rBody['codename']}**.`)
            .addField("Code Link", `https://xottikmwgg.drmizayt.repl.co/code/${kod}`, true)
            .addField("Code Description", rBody['codedesc'], true)
            .addField("Code Category", rBody['category'], true)
        )
